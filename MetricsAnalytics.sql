CREATE STREAM "DESTINATION_SQL_STREAM"( 
    MetricType VARCHAR(16), -- (i.e. event_count, top_pages, visitor_count, page_count...) Corresponds to something you will search on from the UI.  If it is a MAX stock quote for a window, then the Metric would be something like: MAX_AMZN 
    EventTimestamp BIGINT,  -- UNIX timestamp format 
    MetricItem VARCHAR(1024),  -- the list item value for sets.  i.e. (pageview, conversion, exception, click... for event_cont; page uri for top_pages, etc.)
    UnitValueInt BIGINT,  -- If the value is an integer
    UnitValueFloat DOUBLE);  -- If the value is a decimal
...
--"Top" Page Views (Actually all pages with more than one hit in the window, because group_rank not working for me)
...
SELECT 'agent_count', UNIX_TIMESTAMP(eventTimestamp), agent, agent_count FROM (
    SELECT STREAM 
        ...
        STEP (weblogs.ROWTIME BY INTERVAL '1' DAY) as eventTimestamp
    FROM "WASA_001" weblogs
    GROUP BY 
        weblogs."agent",
        STEP (weblogs.ROWTIME BY INTERVAL '1' DAY)
);
